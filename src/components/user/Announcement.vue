...
await proxy.$post(configs.APIS.User.QAnc, {
  'id': id
}, {
  headers: { 'token': token }
}).then((res: any) => {
...
const sendMsg = async () => {
  await proxy.$post(configs.APIS.Admin.SendMsg, msgData, {
    headers: { 'token': token }
  }).then((res: any) => {
    console.log(res)
...
<template>
  ...
  <section class="paper_msg_container">
    <header class="paper_msg_header">
      <h1 class="paper_msg_headline">通知公告</h1>
      <hr>
    </header>
    <article class="msgs" v-if="messages.length">
      <el-collapse accordion @change="notNew">
        <el-collapse-item v-if="identy == 'admin'" disabled class="send_msg">
          <template #title>
            ...
            <el-dialog v-model="sendMsgDia" title="发 送 公 告">
